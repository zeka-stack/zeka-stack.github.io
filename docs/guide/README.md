...
笔者从未将 **Zeka.Stack** 称为一个“框架”，因为在我看来，**只有像 Spring、Play Framework、Micronaut 这类具备更高抽象能力、运行机制设计、强扩展性和通用性
** 项目，才能真正称为**框架**。它们关注的是如何构建通用的开发模式和运行时行为，具有明确的技术范式，并与具体业务逻辑保持解耦。
...
1. 能用: 框架能够正常运行，实现基础功能;

2. 好用: 框架不仅能用，而且使用体验优秀，能提高开发效率;
...
3. 用好: 在理解框架内部机制的基础上，能够发挥框架的最大价值，做到高性能、可扩展、可维护;

   > 这一条针对使用者来说的, 而对于框架开发者来说就需要考虑高内聚低耦合, 易于扩展等特性, 考虑更多的细节和设计.

业务框架的另一个作用是: **提质增效**, 这个就很好理解了:

1. **提质**: 代码质量其实只是一小部分, 还应该包括架构质量, 业务质量等, 通过框架约束和工具支持，减少漏处理、逻辑漏洞或不一致等问题;
2. **增效**: 普遍指开发效率, 但是还应该有运维效率, 协作效率等等支撑型的效率提升, 就拿最简单的说, 框架是否能提供一个通用的部署脚本, 在非
   docker 环境中一件部署.
